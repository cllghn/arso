...
```{r 3-2-weakcomp, fig.cap = "Red con múltiples componentes débiles"}
...
El detectar componentes débiles no solo sirve para colorear los grafos, de hecho, una de las aplicaciones más comunes es el aislar componentes. Por ejemplo, la densidad del grafo en la Figura \@ref(fig:3-2-weakcomp) es `r edge_density(g)`. Este índice sugiere una red escarza, sin embargo, de manera visual podemos ver que algunos componentes son más densos que otros. El componente principal, aquel de mayor tamaño, aparenta ser más denso. Es por ello que podemos aislar este componente principal y al ejecutar la medida de densidad nuevamente vemos un valor de `r g %>% set_vertex_attr(name = "component", value = components(.)$membership) %>% induced_subgraph(which(V(.)$component == 1)) %>% edge_density()`. Este proceso nos permite enfocarnos en diferentes subconjuntos del grafo en nuestro análisis. 
...
```{r 3-2-strongcomp, fig.cap = "Red con múltiples componentes fuertes"}
...
# Complete
g %>%
  my_igraph(.layout = "layout_with_kk", sub = "Red completa",
            .vertex.size = 30)
# 1-core
g %>%
  delete_vertices(v = which(V(.)$coreness < 1)) %>%
  my_igraph(.layout = "layout_with_fr", sub = "1-Core",
            .vertex.size = 30)
```

```{r 3-2-kcore, fig.cap = "Red completa y 1-, 2-, y 3-core", out.width="100%"}
par(mfrow = c(1, 2))
# 2-core
g %>%
  delete_vertices(v = which(V(.)$coreness < 2)) %>%
  my_igraph(.layout = "layout_with_fr", sub = "2-Core",
            .vertex.size = 30)
# 3-core
g %>%
  delete_vertices(v = which(V(.)$coreness < 3)) %>%
  my_igraph(.layout = "layout_with_fr", sub = "3-Core",
            .vertex.size = 30)
```
...
```{r 3-2-bowtie, fig.cap = "Red hipotética (izquierda) y componentes débiles (derecha)"}
par(mfrow = c(1, 2))
...
```{r 3-2-gn, fig.cap="Comunidades Girvan-Newman y puentes resaltados."}
g <- data.frame(from = c("A", "A", "B", "B", "D", "D", "E"),
                to   = c("B", "C", "C", "D", "E", "F", "F")) %>%
  graph_from_data_frame(directed = FALSE) %>%
  set_edge_attr(name = "width", value = edge.betweenness(., directed = FALSE)) %>%
  set_vertex_attr(name = "color", value = membership(cluster_edge_betweenness(.)))
cluster_edge_betweenness(g) %>% plot(., g, vertex.size = 50)
```
...
```{r 3-2-tie3, fig.cap = "Red hipotética"}
g <- data.frame(from = c("A", "A", "B", "B", "D", "D", "E"),
                to   = c("B", "C", "C", "D", "E", "F", "F")) %>%
  graph_from_data_frame(directed = FALSE) 
...
```{r 3-2-process, fig.cap="Visualización de los pasos en el algoritmo", out.width="100%"}
...
g %>%
  igraph::plot.igraph(
    ...
    vertex.size  = 30,
    edges.color  = "grey",
    main         = "Optimización de modularidad")
...
```{r 3-2-bigone, fig.cap="Red sin escala (izquierda) y red sin escala con particiones (derecha)", out.width="100%"}
...
```{r 3-2-1, fig.cap="Reporte de Componentes Conectados en Gephi"}
knitr::include_graphics("images/03-09-01.png")
```

```{r 3-2-2, fig.cap="Red de confianza con actores vivos, color por componente"}
...
```{r 3-2-3, fig.cap="Red de confianza con actores vivos, componente principal"}
...
```{r 3-2-4, fig.cap="Red de confianza con actores vivos, 7-core"}
