...
```{r pandilleros, fig.cap = "Red de Pandilleros"}
files <- list.files(path       = "C:/Users/localuser/Dropbox/arso/data/pandillas/",
                    pattern    = "*.csv$",
                    full.names = TRUE)
...
```{r vivos, fig.cap = "Red de Pandilleros Vivos y Libres"}
g %>%
  delete.vertices(.,
...
```{r fin, fig.cap = "Red de Pandilleros con Lazos Financieros"}
g %>%
  delete.edges(.,
...
```{r pos, fig.cap = "Red de Pandilleros con Nodos Coloreados por Pandilla"}
g %>%
  plot.igraph(
            layout = layout_with_kk,
            vertex.label = NA,
...
```{r collapsed, fig.cap= "Pandilleros, Collapsados por Grupo Ilícito"}
g %>%
  contract(.,
...
```{r importacion, fig.cap= "Cuadro de dialogo para importar un archivo CSV (1 de 2)"}
knitr::include_graphics("images/02-07-01.png")
```

```{r importacion2, fig.cap= "Cuadro de dialogo para importar un archivo CSV (2 de 2)"}
...
```{r importacion3, fig.cap= "Informe de importación de Gephi"}
...
```{r importacion4, fig.cap= "Cuadro de dialogo para importar un archivo CSV"}
...
```{r importacion5, fig.cap = "Reporte de importación"}
...
```{r importacion6, fig.cap = "Cuadro de diálogo 'Generar paleta'"}
...
```{r 7, fig.cap = "Red operacional de Noordin"}
...
```{r 9, fig.cap = "Red colapsada por atributo de rol (diseño Force Atlas)"}
...
```{r 10, fig.cap = "Red colapsada por el atributo de rol (diseño OpenOrd)"}
...
```{r 11, fig.cap = "Red viva y libre filtrada"}
...
```{r 12, fig.cap = "Red viva y libre (sin aislados)"}
