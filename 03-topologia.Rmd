...
```{r 3-1-star, fig.cap="Red estrella", dev = 'pdf'}
...
```{r 3-1-full, fig.cap="Red completa", dev = 'pdf'}
...
```{r 3-1-g5, fig.cap="Grafo de red, tamaño = 5"}
igraph::barabasi.game(n = 5, power = 0.5, directed = FALSE) %>%
  my_igraph()
```

```{r 3-1-g10, fig.cap="Grafo de red, tamaño = 10"}
...
```{r 3-1-circ, fig.cap="Grafo circular"}
...
```{r 3-1-diam, fig.cap= "Grafo ejemplo"}
...
```{r 3-1-den, fig.cap= "Grafo no dirigido"}
...
Donde $N$ es el número de nodos y $E$ es el número de vínculos en la red. En términos prácticos, esta ecuación retorna un número promedio de vínculos para todos los actores en la red. Esto nos ayuda a comparar redes de diferentes tamaños. Por ejemplo, las redes \@ref(fig:3-1-avg1) y \@ref(fig:3-1-avg2) tienen un grado medio similar, de `r g100 %>% degree() %>% mean()` y `r g200 %>% degree() %>% mean()` respectivamente. La densidad es equivalente a `r g100 %>% edge_density()` y a `r g200 %>% edge_density()`.

```{r 3-1-avg1, fig.cap= "Grafo sin escala de 100 nodos"}
g100 %>%
  plot.igraph(
    vertex.size = 5,
    vertex.label = NA,
...
```{r 3-1-avg2, fig.cap= "Grafo sin escala de 200 nodos"}
 g200 %>%
  plot.igraph(
    vertex.size = 5,
    vertex.label = NA,
...
```{r 3-1-star3, fig.cap="Red estrella modificada"}
...
```{r 3-1-1, fig.cap="Ventana de descripción general, ventanilla de contexto"}
...
```{r 3-1-2, fig.cap="Ventana de descripción general, pestaña Estadísticas"}
...
```{r 3-1-3, fig.cap="Ventana de descripción general, pestaña Estadísticas"}
