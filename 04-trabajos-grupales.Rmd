...
```{r, echo = FALSE, out.width = "100%", fig.cap = "Red Calor-Calor"}
library(igraph)
...
:::{.rcolumn-70}
```{r}
bbox <- sf::st_bbox(countries[countries[["admin"]] %in% c("Peru", "Colombia", "Nicaragua", "Brazil"), ])
ggplot(data = countries) +
  geom_sf(fill = "antiquewhite") +
  geom_sf(data = countries[countries[["admin"]] == "Peru", ], fill = "tomato") +
  annotation_scale(location = "bl", width_hint = 0.5) +
  annotation_north_arrow(location = "bl", which_north = "true", 
...
```{r 4-3-grafo, fig.cap = "Datos modo-uno agregados, nodos por intermediación", out.width = "100%"}
el <- read.csv("student_data/consolidated.csv")[, c("Source", "Target", "Weight")]
names(el) <- c("Source", "Target", "weight")
nl <- read.csv("student_data/consolidated_nodes.csv")

g <- graph_from_data_frame(d = el,
                           directed = FALSE, 
                           vertices = nl) %>%
  simplify(remove.multiple = TRUE,
           edge.attr.comb = "sum") %>%
  set.vertex.attribute(name = "label", value = sapply(1:length(V(.)),
                                                      function(s) {paste0("A",
                                                                          s)})) %>%
  set.vertex.attribute(name = "degree", value = degree(g)) %>%
  set.vertex.attribute(name = "betweenness", value = betweenness(g, 
                                                                 directed = FALSE,
                                                                 normalized = TRUE))
...
cols <- c("#7fc97f", "#beaed4", "#fdc086", "#ffff99", "#386cb0", "#f0027f", 
          "#bf5b17")
...
plot(g, 
     ...
     vertex.label.size = 5,
     vertex.size = scales::rescale(betweenness(g), to = c(12, 25)),
     vertex.color = cols[as.factor(V(g)$pais)], 
...
legend("bottomright",
       bty = "n",
       legend = c("Argentina", "Bolivia", "Brasil", "Ecuador", "Irán", "Perú",
                  "Venezuela"), 
       fill = c("#7fc97f", "#beaed4", "#fdc086", "#ffff99", "#386cb0", "#f0027f", 
                "#bf5b17"),
       border = NA)
```
